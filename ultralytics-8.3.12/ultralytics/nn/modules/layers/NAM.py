...
__all__ = ["NAMAttention", "C3_NAM", "C2f_NAM"]
...
class C3_NAM(nn.Module):
    # CSP Bottleneck with 3 convolutions and 1 NAM. by CSDN迪菲赫尔曼
    def __init__(
        ...
        self.m = nn.Sequential(
            *(NAM_Bottleneck(c_, c_, shortcut, g, e=1.0) for _ in range(n))
        )
...
class NAM_Bottleneck_(nn.Module):
    def __init__(
...
class C2f_NAM(nn.Module):
    ...
    def __init__(
        ...
        self.m = nn.ModuleList(
            NAM_Bottleneck_(self.c, self.c, shortcut, g, k=((3, 3), (3, 3)), e=1.0)
            for _ in range(n)
        )
